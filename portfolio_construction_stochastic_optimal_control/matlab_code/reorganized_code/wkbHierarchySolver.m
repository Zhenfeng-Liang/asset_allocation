    ...
    methods
        ...
        function [S] = calcS(obj, x, t, T, timeStep, tol)
            ...
            for i = 1:(size(xPath, 2)/2 - 1)   % Note the original R code is different here
                ...
                S1 = S1 + trace(obj.hamSys.port.instCov(xPath(:,2*i-1)) * dpDx(:,:,2*i-1) ...
                    + 4 * obj.hamSys.port.instCov(xPath(:,2*i)) * dpDx(:,:,2*i) ...
                    + obj.hamSys.port.instCov(xPath(:,2*i+1)) * dpDx(:,:,2*i+1));
        ...
        function [val] = lagr(obj, x, p)
            ...
            a = obj.hamSys.port.model.driftV(x);
            
            term1 = 0.5 * obj.hamSys.oneOverGamma * p' * obj.hamSys.port.instCov(x) * p;
            term2 = 0.5 * obj.hamSys.kappa * a' * obj.hamSys.port.invInstCov(x) * a;
            val = term2 - term1;
